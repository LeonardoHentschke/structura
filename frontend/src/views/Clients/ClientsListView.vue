...
const hasLinkedProjects = ref(false);
...
const openDeleteModal = async (clientId) => {
  ...
  try {
    const { hasProjects, projects } = await projectStore.checkProjects(clientId);
    hasLinkedProjects.value = hasProjects;
    linkedProjects.value = projects;
  } catch (error) {
    console.error("Erro ao verificar projetos vinculados:", error.message);
    hasLinkedProjects.value = false;
...
const deleteClient = async () => {
  if (!hasLinkedProjects.value && selectedClientId.value !== null) {
    try {
      const success = await clientsStore.deleteClient(selectedClientId.value);
      if (success) {
        isModalOpen.value = false;
        selectedClientId.value = null;
        await clientsStore.getAllClients(); // Atualizar lista de clientes
        closeDeleteModal();
      } else {
...
<template>
  <div class="min-h-screen flex-1 flex-col space-y-8 p-8 md:flex dark:bg-gray-900">
    <div class="flex items-center justify-between space-y-2">
      <div>
        <h2 class="text-3xl font-extrabold tracking-tighter text-primary dark:text-yellow-500">
          Sua lista de clientes
        </h2>
        <p class="text-muted-foreground text-sm tracking-wide dark:text-gray-300">
          Gerencie seus clientes cadastrados facilmente
        </p>
      </div>
    ...
    <div class="bg-card dark:bg-gray-800 p-6 rounded-lg shadow-xl">
      <Table class="w-full text-left bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700">
        <TableCaption v-if="!clientsStore.clients.length" class="text-muted-foreground dark:text-gray-400">
        ...
        <TableBody>
          <TableRow
            ...
            :key="client.id"
            class="hover:bg-accent/30 dark:hover:bg-gray-700 transition-colors duration-200"
          >
        ...
        <div class="max-w-xl w-full mx-auto bg-gray-800 rounded-xl overflow-hidden">
          <div class="max-w-sm mx-auto pt-12 pb-8 px-5 text-center">
            ...
            <h4 class="text-xl text-gray-100 font-semibold mb-5">
              {{ hasLinkedProjects ? "Não é possível excluir" : "Deseja excluir este cliente?" }}
            </h4>
            <p class="text-gray-300 font-medium">
              {{ hasLinkedProjects ? "Existem projetos vinculados a este cliente:" : "Você pode excluir permanentemente este cliente." }}
            </p>
            <ul v-if="hasLinkedProjects" class="mt-4 text-gray-300">
            <li v-for="project in linkedProjects" :key="project.id">{{ project.id }} - {{ project.name }}</li>
          </ul>
          <div class="mt-6">
            <Button @click="closeDeleteModal">Cancelar</Button>
            <Button v-if="!hasLinkedProjects" @click="deleteClient" class="btn-danger">Excluir</Button>
          </div>
        </div>
      </div>
    </div>

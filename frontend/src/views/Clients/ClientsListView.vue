...
const isMenuOpen = ref({}); // Objeto de controle de estado para cada cliente
...
const checkProjects = async (clientId) => {
  ...
  // hasProjects.value = await clientsStore.hasProjects(clientId);
};

const selectedClientId = ref(null);

const openDeleteModal = async (clientId) => {
  selectedClientId.value = clientId; // Armazena o ID do cliente selecionado
  await checkProjects(clientId);
  isModalOpen.value = true;
};

const deactivateClient = async () => {
  if (selectedClientId.value !== null) {
    await clientsStore.deactivateClient(selectedClientId.value); // Chame a função de desativação usando o ID armazenado
...
<template>
  <div class="h-full flex-1 flex flex-col space-y-8 p-8 bg-gray-100 dark:bg-gray-900">
    <!-- Header -->

...
<template>
  <div class="min-h-screen flex-1 flex-col space-y-8 p-8 md:flex dark:bg-gray-900">
    <div class="flex items-center justify-between space-y-2">
      <div>
        <h2 class="text-3xl font-extrabold tracking-tighter text-primary dark:text-yellow-500">Lista de Funcionários</h2>
        <p class="text-muted-foreground text-sm tracking-wide dark:text-gray-300">Gerencie seus funcionários cadastrados facilmente</p>
      </div>
      <div class="flex items-center space-x-2">
        <Button @click="() => router.push({ name: 'employeeCreate' })" class="bg-primary text-primary-foreground dark:bg-yellow-500 dark:text-gray-800 font-bold transition-colors duration-300 shadow-md hover:shadow-lg">
    ...
    <div class="bg-card dark:bg-gray-800 p-6 rounded-lg shadow-xl">
      <Table class="w-full text-left bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700">
        <TableCaption v-if="!employees.length" class="text-muted-foreground dark:text-gray-400">
        ...
        <TableBody>
          <TableRow v-for="employee in employees" :key="employee.id" class="hover:bg-accent/30 dark:hover:bg-gray-700 transition-colors duration-200">
            <TableCell class="py-4 px-4 text-center dark:text-gray-300">{{ employee.id }}</TableCell>
            <TableCell class="font-medium py-4 px-4 dark:text-gray-100">{{ employee.name }}</TableCell>
            <TableCell class="py-4 px-4 dark:text-gray-300">{{ employee.email }}</TableCell>
            <TableCell class="py-4 px-4 dark:text-gray-300">{{ employee.phone }}</TableCell>
            <TableCell class="py-4 px-4 dark:text-gray-300">{{ employee.cpf }}</TableCell>
            <TableCell class="flex justify-center items-center relative" @mouseleave="closeMenu(employee.id)">
              <!-- Botão de Menu (Três Pontos) -->
              <template v-if="!isMenuOpen[employee.id]">
                <Button
              ...
              <template v-else>
                <div class="flex space-x-3 items-center">
                  <!-- Botão Editar -->
                  <Button variant="ghost" class="editBtn" @click="handleEdit(employee.id)">
                  ...
                  <!-- Botão Excluir -->
                  <button @click="handleDelete(employee.id)" class="group relative flex h-10 w-10 flex-col items-center justify-center overflow-hidden rounded-xl border-2 border-red-800 bg-red-400 hover:bg-red-600"
                    title="Excluir">

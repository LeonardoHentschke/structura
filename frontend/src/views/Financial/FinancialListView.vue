...
const selectedFinancialId = ref(null);
const openDeleteModal = async (financialId) => {
  isDeleteModalOpen.value = true;
  selectedFinancialId.value = financialId;
  isModalOpen.value = true;
};
// Fecha o modal sem excluir
const closeDeleteModal = () => {
  isModalOpen.value = false; // Fecha o modal principal
  isDeleteModalOpen.value = false; // Garante que o modal de exclusão é fechado
  selectedFinancialId.value = null; // Reseta o ID selecionado
};
...
const deleteFinance = async (id) => {
  try {
    // Chama a função do store para deletar a movimentação financeira
    await financialStore.deleteFinance(id);

    // Fecha o modal após a exclusão
    closeDeleteModal();

    // Atualiza as movimentações financeiras após exclusão
    await financialStore.getAllFinances();
    ...
    <div class="flex items-center justify-between space-y-2">
      ...
      <div class="flex items-center space-x-4">
        <div class="flex items-center space-x-2">
          ...
          <select v-model="selectedProjectId" @change="filterByProject" class="input w-full max-w-xs dark:bg-gray-800 p-2 shadow-md rounded-lg nav-button">
      ...
      <Table class="w-full text-left bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700">
        ...
        <TableBody>
          ...
          <TableRow v-for="finance in filteredFinances" :key="finance.id" class="hover:bg-accent/30 dark:hover:bg-gray-700">
            ...
            <TableCell class="flex justify-center items-center relative" @mouseleave="closeMenu(finance.id)">

              <template v-if="!isMenuOpen[finance.id]">
                <Button
                  @click="toggleMenu(finance.id)"
                  variant="ghost"
              ...
              <template v-else>
                <div class="flex space-x-3 items-center">
                  <!-- Botão Editar -->
                  <RouterLink :to="{ name: 'financial', params: { id: finance.id } }" class="editBtn">
                  ...
                  <!-- Botão Excluir -->
                  <button @click="openDeleteModal(finance.id)" class="group relative flex h-10 w-10 flex-col items-center justify-center overflow-hidden rounded-xl border-2 border-red-800 bg-red-400 hover:bg-red-600"
                    title="Excluir">
    ...
    <div v-if="isModalOpen" class="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50">
      <div class="max-w-xl w-full mx-auto bg-gray-800 rounded-xl overflow-hidden">
        ...
        <div class="flex justify-center pt-5 pb-6 px-6 text-right bg-gray-900 -mb-2">
          ...
          <button @click="deleteFinance(selectedFinancialId)" class="inline-block w-full sm:w-auto py-3 px-5 mb-2 text-center font-semibold leading-6 text-blue-50 bg-red-500 hover:bg-red-600 rounded-lg transition duration-200">
            Deletar
          </button>
        </div>
      </div>
